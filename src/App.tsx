...
function App() {
  ...
  const taskTabRef = useRef<HTMLButtonElement | null>(null)
  ...
  function handleTabKeyDown(event: KeyboardEvent<HTMLButtonElement>) {
    ...
    const nextTab = event.key === 'Home' ? 'task' : event.key === 'End' ? 'workspace' : mobileTab === 'task' ? 'workspace' : 'task'
    setMobileTab(nextTab)
    if (nextTab === 'task') taskTabRef.current?.focus()
    else workspaceTabRef.current?.focus()
  }
  ...
  const progress = attempt.completedAt ? 100 :
    (attempt.plan.trim() ? 25 : 0) +
    (attempt.code !== rep.starter ? 25 : 0) +
    (allPassed ? 25 : 0) +
    (canFinish ? 15 : 0)
  ...
  return <div className="app-shell">
    ...
    {view === 'home' ? <main className="home-main" id="top">
      ...
      <div className="page-intro"><div className="breadcrumb">PRACTICE <span>/</span> {rep.category.toUpperCase()}</div><div className="intro-row"><div><h1>{rep.title}</h1><p className="intro-copy">A small problem, a clear plan, and one more skill you can trust.</p></div></div><label className="rep-picker-label" htmlFor="rep-picker">CHOOSE A REP</label><select id="rep-picker" className="rep-picker" value={repId} onChange={(event) => selectRep(event.target.value)}>{reps.map((item, index) => <option key={item.id} value={item.id}>{String(index + 1).padStart(2, '0')} · {item.title}</option>)}</select><div className="progress-track" role="progressbar" aria-label="Rep progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><span style={{ width: `${progress}%` }} /></div></div>
      <div className="mobile-tabs" role="tablist" aria-label="Rep panels"><button ref={taskTabRef} id="task-tab" type="button" role="tab" aria-controls="task-panel" aria-selected={mobileTab === 'task'} tabIndex={mobileTab === 'task' ? 0 : -1} onKeyDown={handleTabKeyDown} onClick={() => setMobileTab('task')}>Task &amp; plan</button><button ref={workspaceTabRef} id="workspace-tab" type="button" role="tab" aria-controls="workspace-panel" aria-selected={mobileTab === 'workspace'} tabIndex={mobileTab === 'workspace' ? 0 : -1} onKeyDown={handleTabKeyDown} onClick={() => setMobileTab('workspace')}>Workspace</button></div>
      <div className="workspace-layout">
        <div id="task-panel" role="tabpanel" aria-labelledby="task-tab" className={`task-column ${mobileTab === 'workspace' ? 'mobile-hidden' : ''}`}>
        ...
        <div id="workspace-panel" role="tabpanel" aria-labelledby="workspace-tab" className={`code-column ${mobileTab === 'task' ? 'mobile-hidden' : ''}`}>
